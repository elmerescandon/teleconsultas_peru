...
const useDoctorRegister = () => {
    ...
    const validations = {
        ...
        cmpId: (value: string) => (value ? '' : 'Inserta tu número de colegiatura.'),
    ...
    const handleValidations = (type: 'general' | 'location' | 'info') => {
        ...
        const infoField = ["age", "specialities", "cmpNumber", "curriculum", "phone", "bornDate", "termsAndConditions", "cmpId"] as (keyof IFormFields)[];
        switch (type) {
            case 'general':
