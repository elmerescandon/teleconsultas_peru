...
const LinkPrimary = ({ children, to, type, color }: LinkPrimaryProps) => {
    ...
    return (
        <Link
            ...
            } rounded-lg px-6 font-semibold py-4 text-center hover:opacity-80 transition-opacity bg-opacity-80 max-xl:text-sm max-lg:py-3 max-md:text-2xl max-md:px-5 max-md:py-5`}
        >
            {children}
        </Link>
    );
