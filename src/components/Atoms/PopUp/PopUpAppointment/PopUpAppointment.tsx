    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      ...
      >
        <div className="flex justify-between items-center">
          <h2 className="text-3xl font-semibold">{summary.specialityName} </h2>
        ...
        <div className="flex flex-col">
          <LabelInformation label="Doctor" value={summary.doctorName} />
          ...
          />
          <LabelInformation label="Razón" value={`${appointment.reason}`} />
        </div>
        {appointment.status === "pending" && (
          <Link
        ...
        {(appointment.status === "pending" ||
          appointment.status === "scheduled") &&
          isDateOlderThan24Hours(appointment.date as unknown as Timestamp) && (
            <button
