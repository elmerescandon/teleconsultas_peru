...
import createAvailabilitySlots from "@/firebase/Availability/createAvailabilitySlots";
...
const AddAvailabilityGeneral = ({
  ...
  const addNewSchedule = async () => {
    try {
      ...
      if (repeat === "never") {
        await createAvailabilitySlots(dateInput, specialityId, doctorId, slots);
      } else if (
        repeat === "daily" ||
        repeat === "weekly" ||
        repeat === "monthly"
        ...
        const datesRange = getWeekdaysDatesBetween(date, endDate, repeat);
        ...
        if (startTime && endTime) {
          datesRange.map(async (date) => {
            ...
            const dateInput = setDateToTimezoneConstant(
      ...
      setError((error as Error).message || "Error al agregar disponibilidad");
      setUploaded(false);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="relative flex flex-col gap-5">
        {!check && <HourAvailability title="Inicio" setTime={setStartTime} />}
        {!check && <HourAvailability title="Termina" setTime={setEndTime} />}

        <RepeatAvailability title={"Se repite"} setRepeatability={setRepeat} />
        {repeat !== "never" && repeat !== "" && (
          <DateAvailability
            title="Hasta"
            date={endDate}
      ...
      <div className="py-5">
        <ButtonPrimary onClickFn={addNewSchedule}>Agregar</ButtonPrimary>
        {loading && <LoadingHorizontal />}
      </div>
      {error && <p className="text-red-500 font-semibold py-5">{error}</p>}
      {uploaded && (
